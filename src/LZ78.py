...
def encode(txt):
    ...
    bs = 0
    bitsPerLetter = Utils.getNumbersOfBitsPerLetter(txt)

    word = ""
    while len(txt) > 0:

        #LOOK FOR LARGEST PREFIX
        tupl = Utils.getLargestPrefixPointer(txt, dic)
        ...
        bs = bs + Utils.getNumberOfBitsPerTokenLZ78(pointer, bitsPerLetter)
...
def decode(tokens):
    ...
    for tok in tokens:
        ...
        word = Utils.findKeyOfValue(dic, pointer)
        if(word == None):
            newWord =  c
        else:
            newWord = word + c
