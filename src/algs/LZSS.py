...
def encode(txt, s, t, m):
    ...
    bs = 0
    numberBitsSimple = Utils.getNumbersOfBitsPerLetter(txt) + 1
    ...
    while len(lookahead) > 0:
        ...
        while(len(tmpSubstring) >= m):
            ...
            index = Utils.findSubstring(searchBuffer, tmpSubstring)
            if(index != -1):
        ...
        if(flagBit == 0):
            ...
            bs = bs + Utils.getNumbersBitsPerTokenLZSS(offset, length)
